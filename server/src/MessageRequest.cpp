...
int MessageRequest::fetch_websocket_info(char *msg){
	int pos = 0;
	fetch_fin(msg, pos);
...
int MessageRequest::fetch_fin(char *msg, int &pos){
	fin_ = (unsigned char)msg[pos] >> 7;
	return 0;
}

int MessageRequest::fetch_opcode(char *msg, int &pos){
	opcode_ = msg[pos] & 0x0f;
	pos++;
	return 0;
}

int MessageRequest::fetch_mask(char *msg, int &pos){
	mask_ = (unsigned char)msg[pos] >> 7;
	return 0;
}

int MessageRequest::fetch_masking_key(char *msg, int &pos){
	if(mask_ != 1)
		return 0;
	for(int i = 0; i < 4; i++)
...
int MessageRequest::fetch_payload_length(char *msg, int &pos){
...
int MessageRequest::fetch_payload(char *msg, int &pos){
	memset(payload_, 0, sizeof(payload_));
	if(mask_ != 1){
...
char* MessageRequest::get_payload() {
